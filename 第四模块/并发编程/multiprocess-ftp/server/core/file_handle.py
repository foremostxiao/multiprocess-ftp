...
class FileHandle():
    ...
    def recursion_file(self, menu):
        """递归查询用户home/alex目录下的所有文件，算出文件的大小"""
        res = os.listdir(menu)
        for i in res:
            path =os.path.join(menu, i)
    ...
    def verification_filemd5(self,conn_obj, file_md5, conn):
        ...
        if self.getfile_md5(conn_obj['filepath']) == file_md5:
            conn.send(self.message['211'])
            # 上传成功
        else:
            conn.send(self.message['212'])
            # 上传失败
    ...
    def write_file(self, f, recv_size, file_size, conn):
        ...
        while recv_size < file_size:
            ...
            conn.send(struct.pack('i', recv_size))
            # 为了进度条的显示
